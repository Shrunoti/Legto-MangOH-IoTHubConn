...
{
    int result;
    LogError("PSOC6---DEBUGGING_amqp_management_open_async---PSOC6");
    ...
    else if (amqp_management->amqp_management_state != AMQP_MANAGEMENT_STATE_IDLE)
    {
        ...
        LogError("PSOC6---AMQP management instance already OPEN---PSOC6");
        result = __FAILURE__;
    }
    else
    {
        ...
        if (messagereceiver_open(amqp_management->message_receiver, on_message_received, amqp_management) != 0)
        {
            ...
            LogError("PSOC6---Failed opening message receiver---PSOC6");
            amqp_management->amqp_management_state = AMQP_MANAGEMENT_STATE_IDLE;
            result = __FAILURE__;
        }
        else
        {
            ...
            if (messagesender_open(amqp_management->message_sender) != 0)
            {
                ...
                LogError("PSOC6---Failed opening message sender---PSOC6");
                amqp_management->amqp_management_state = AMQP_MANAGEMENT_STATE_IDLE;
            ...
            {
                LogError("PSOC6---Opening_message_sender_successfully---PSOC6");
...
int amqp_management_close(AMQP_MANAGEMENT_HANDLE amqp_management)
{
    ...
    else if (amqp_management->amqp_management_state == AMQP_MANAGEMENT_STATE_IDLE)
    {
        ...
        LogError("PSOC6---AMQP management instance not open---PSOC6");
        result = __FAILURE__;
    }
    else
    {
