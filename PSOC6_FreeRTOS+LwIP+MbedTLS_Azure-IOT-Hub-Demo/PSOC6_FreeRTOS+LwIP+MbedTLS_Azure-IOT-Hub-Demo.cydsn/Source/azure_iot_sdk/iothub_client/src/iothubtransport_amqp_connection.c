...
static void on_cbs_open_complete(void* context, CBS_OPEN_COMPLETE_RESULT open_complete_result)
{
    ...
    {
        LogError("PSOC6---CBS open failed---PSOC6");
    }
}
...
static int create_cbs_handle(AMQP_CONNECTION_INSTANCE* instance)
{
    ...
    if ((instance->cbs_handle = cbs_create(instance->session_handle)) == NULL)
    {
        ...
        result = __FAILURE__;
        LogError("PSOC6---Failed to create the CBS connection---PSOC6");
    }
    ...
    else if (cbs_open_async(instance->cbs_handle, on_cbs_open_complete, instance->cbs_handle, on_cbs_error, instance->cbs_handle) != RESULT_OK)
    {
        ...
        result = __FAILURE__;
        LogError("PSOC6---Failed to open the connection with CBS---PSOC6");
    }
    else
    {
        result = RESULT_OK;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    LogError("PSOC6---amqp_connection_create_config_create_csb_connection_is_TRUE---PSOC6"); //AMQP has CBS connection
                }
                ...
                {
                    result = NULL;
                    LogError("PSOC6---amqp_connection_create failed (failed creating the SASL components)---PSOC6");
                }
                ...
                {
                    result = NULL;
                    LogError("PSOC6---amqp_connection_create failed (failed creating the CBS handle)---PSOC6");
                }
                else
                {
