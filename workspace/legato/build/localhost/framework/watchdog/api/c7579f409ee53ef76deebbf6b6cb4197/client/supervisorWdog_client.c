...
#include "supervisorWdog_messages.h"
#include "supervisorWdog_interface.h"
...
typedef struct
{
    ...
    supervisorWdog_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "supervisorWdog_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("supervisorWdog_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("supervisorWdog_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("supervisorWdog_ClientHandlers", 5);
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void supervisorWdog_ConnectService
(
    void
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t supervisorWdog_TryConnectService
(
    void
)
{
...
)
{
    ...
    LE_FATAL("Component for supervisorWdog disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void supervisorWdog_SetServerDisconnectHandler
(
    supervisorWdog_DisconnectHandler_t disconnectHandler,
    void *contextPtr
)
{
    _ClientThreadData_t* clientThreadPtr = GetClientThreadDataPtr();
 ...
 */
//--------------------------------------------------------------------------------------------------
void supervisorWdog_DisconnectService
(
    void
)
{
...
static void _Handle_supervisorWdog_AddKickEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    supervisorWdog_KickHandlerFunc_t _handlerRef_supervisorWdog_AddKickEventHandler = (supervisorWdog_KickHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;

    // Unpack the remaining parameters

    // Call the registered handler
    if ( _handlerRef_supervisorWdog_AddKickEventHandler != NULL )
    {
        _handlerRef_supervisorWdog_AddKickEventHandler(contextPtr );
    }
    else
    {
...
)
{
    ...
    supervisorWdog_KickEventHandlerRef_t _result;
    ...
    _msgPtr->id = _MSGID_supervisorWdog_AddKickEventHandler;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_supervisorWdog_RemoveKickEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (supervisorWdog_KickEventHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_supervisorWdog_AddKickEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_supervisorWdog_AddKickEventHandler, msgRef, clientDataPtr);
            break;
