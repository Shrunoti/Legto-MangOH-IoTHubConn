...
#include "logDaemonWdog_messages.h"
#include "logDaemonWdog_interface.h"
...
typedef struct
{
    ...
    logDaemonWdog_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "logDaemonWdog_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("logDaemonWdog_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("logDaemonWdog_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("logDaemonWdog_ClientHandlers", 5);
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void logDaemonWdog_ConnectService
(
    void
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t logDaemonWdog_TryConnectService
(
    void
)
{
...
)
{
    ...
    LE_FATAL("Component for logDaemonWdog disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void logDaemonWdog_SetServerDisconnectHandler
(
    logDaemonWdog_DisconnectHandler_t disconnectHandler,
    void *contextPtr
)
{
    _ClientThreadData_t* clientThreadPtr = GetClientThreadDataPtr();
 ...
 */
//--------------------------------------------------------------------------------------------------
void logDaemonWdog_DisconnectService
(
    void
)
{
...
static void _Handle_logDaemonWdog_AddKickEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    logDaemonWdog_KickHandlerFunc_t _handlerRef_logDaemonWdog_AddKickEventHandler = (logDaemonWdog_KickHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;

    // Unpack the remaining parameters

    // Call the registered handler
    if ( _handlerRef_logDaemonWdog_AddKickEventHandler != NULL )
    {
        _handlerRef_logDaemonWdog_AddKickEventHandler(contextPtr );
    }
    else
    {
...
)
{
    ...
    logDaemonWdog_KickEventHandlerRef_t _result;
    ...
    _msgPtr->id = _MSGID_logDaemonWdog_AddKickEventHandler;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_logDaemonWdog_RemoveKickEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (logDaemonWdog_KickEventHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_logDaemonWdog_AddKickEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_logDaemonWdog_AddKickEventHandler, msgRef, clientDataPtr);
            break;
