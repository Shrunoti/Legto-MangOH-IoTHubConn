...
#include "updateDaemonWdog_messages.h"
#include "updateDaemonWdog_interface.h"
...
typedef struct
{
    ...
    updateDaemonWdog_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "updateDaemonWdog_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("updateDaemonWdog_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("updateDaemonWdog_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("updateDaemonWdog_ClientHandlers", 5);
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void updateDaemonWdog_ConnectService
(
    void
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t updateDaemonWdog_TryConnectService
(
    void
)
{
...
)
{
    ...
    LE_FATAL("Component for updateDaemonWdog disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void updateDaemonWdog_SetServerDisconnectHandler
(
    updateDaemonWdog_DisconnectHandler_t disconnectHandler,
    void *contextPtr
)
{
    _ClientThreadData_t* clientThreadPtr = GetClientThreadDataPtr();
 ...
 */
//--------------------------------------------------------------------------------------------------
void updateDaemonWdog_DisconnectService
(
    void
)
{
...
static void _Handle_updateDaemonWdog_AddKickEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    updateDaemonWdog_KickHandlerFunc_t _handlerRef_updateDaemonWdog_AddKickEventHandler = (updateDaemonWdog_KickHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;

    // Unpack the remaining parameters

    // Call the registered handler
    if ( _handlerRef_updateDaemonWdog_AddKickEventHandler != NULL )
    {
        _handlerRef_updateDaemonWdog_AddKickEventHandler(contextPtr );
    }
    else
    {
...
)
{
    ...
    updateDaemonWdog_KickEventHandlerRef_t _result;
    ...
    _msgPtr->id = _MSGID_updateDaemonWdog_AddKickEventHandler;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_updateDaemonWdog_RemoveKickEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (updateDaemonWdog_KickEventHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_updateDaemonWdog_AddKickEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_updateDaemonWdog_AddKickEventHandler, msgRef, clientDataPtr);
            break;
