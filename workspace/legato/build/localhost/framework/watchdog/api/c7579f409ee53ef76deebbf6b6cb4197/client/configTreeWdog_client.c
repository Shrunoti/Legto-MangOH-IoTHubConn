...
#include "configTreeWdog_messages.h"
#include "configTreeWdog_interface.h"
...
typedef struct
{
    ...
    configTreeWdog_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "configTreeWdog_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("configTreeWdog_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("configTreeWdog_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("configTreeWdog_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for configTreeWdog disconnected\n");
}
...
static void _Handle_configTreeWdog_AddKickEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    configTreeWdog_KickHandlerFunc_t _handlerRef_configTreeWdog_AddKickEventHandler = (configTreeWdog_KickHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;

    // Unpack the remaining parameters

    // Call the registered handler
    if ( _handlerRef_configTreeWdog_AddKickEventHandler != NULL )
    {
        _handlerRef_configTreeWdog_AddKickEventHandler(contextPtr );
    }
    else
    {
...
)
{
    ...
    configTreeWdog_KickEventHandlerRef_t _result;
    ...
    _msgPtr->id = _MSGID_configTreeWdog_AddKickEventHandler;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_configTreeWdog_RemoveKickEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (configTreeWdog_KickEventHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_configTreeWdog_AddKickEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_configTreeWdog_AddKickEventHandler, msgRef, clientDataPtr);
            break;
