...
#include "le_wdog_messages.h"
#include "le_wdog_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_wdog_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_wdog_ServerHandlers", 3);
...
static void Handle_le_wdog_Kick
(
...
static void Handle_le_wdog_Timeout
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_wdog_GetWatchdogTimeout
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_wdog_GetWatchdogTimeout ( 
        millisecondsPtr );
...
static void Handle_le_wdog_GetMaxWatchdogTimeout
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_wdog_GetMaxWatchdogTimeout ( 
        millisecondsPtr );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_wdog_Kick : Handle_le_wdog_Kick(msgRef); break;
        case _MSGID_le_wdog_Timeout : Handle_le_wdog_Timeout(msgRef); break;
        case _MSGID_le_wdog_GetWatchdogTimeout : Handle_le_wdog_GetWatchdogTimeout(msgRef); break;
        case _MSGID_le_wdog_GetMaxWatchdogTimeout : Handle_le_wdog_GetMaxWatchdogTimeout(msgRef); break;
