...
#include "le_appRemove_messages.h"
#include "le_appRemove_interface.h"
...
typedef struct
{
    ...
    le_appRemove_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_appRemove_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_appRemove_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_appRemove_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_appRemove_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_appRemove disconnected\n");
}
...
)
{
    ...
    _msgPtr->id = _MSGID_le_appRemove_Remove;
