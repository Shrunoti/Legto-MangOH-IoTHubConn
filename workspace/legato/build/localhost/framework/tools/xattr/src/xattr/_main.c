...
// Startup code for the executable 'xattr'.
...
LE_SHARED le_log_SessionRef_t xattr_exe_LogSession;
LE_SHARED le_log_Level_t* xattr_exe_LogLevelFilterPtr;

// Declare default component's COMPONENT_INIT function.
void _xattr_exe_COMPONENT_INIT(void);
...
int main(int argc, char* argv[])
{
    ...
    xattr_exe_LogSession = log_RegComponent("xattr_exe", &xattr_exe_LogLevelFilterPtr);
...
// Queue the default component's COMPONENT_INIT to Event Loop.
    event_QueueComponentInit(_xattr_exe_COMPONENT_INIT);
