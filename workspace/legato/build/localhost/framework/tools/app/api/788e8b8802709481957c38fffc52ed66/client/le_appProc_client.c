...
#include "le_appProc_interface.h"
...
typedef struct
{
    ...
    le_appProc_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_appProc_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_appProc_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_appProc_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_appProc_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_appProc disconnected\n");
}
...
)
{
    ...
    if ( strnlen(execPath, 511) > 511 )
    {
        LE_FATAL("strnlen(execPath, 511) > 511");
    }
    ...
    _msgPtr->id = _MSGID_le_appProc_Create;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_SetStdIn;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_SetStdOut;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_SetStdErr;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
static void _Handle_le_appProc_AddStopHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_appProc_StopHandlerFunc_t _handlerRef_le_appProc_AddStopHandler = (le_appProc_StopHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_le_appProc_AddStopHandler != NULL )
    {
        _handlerRef_le_appProc_AddStopHandler(exitCode, contextPtr );
    }
    else
    {
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_RemoveStopHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (le_appProc_StopHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(arg, 511) > 511 )
    {
        LE_FATAL("strnlen(arg, 511) > 511");
    }
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_AddArg;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_ClearArgs;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(priority, 6) > 6 )
    {
        LE_FATAL("strnlen(priority, 6) > 6");
    }
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_SetPriority;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_ClearPriority;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_SetFaultAction;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_ClearFaultAction;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_Start;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_appProc_Delete;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_appProc_AddStopHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_le_appProc_AddStopHandler, msgRef, clientDataPtr);
            break;
