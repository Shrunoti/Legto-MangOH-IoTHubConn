...
#include "configTreeWdog_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("configTreeWdog_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("configTreeWdog_ServerHandlers", 3);
...
static void AsyncResponse_configTreeWdog_AddKickEventHandler
(
    void* contextPtr
)
{
...
static void Handle_configTreeWdog_AddKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = configTreeWdog_AddKickEventHandler ( 
        interval, AsyncResponse_configTreeWdog_AddKickEventHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)configTreeWdog_RemoveKickEventHandler;
...
static void Handle_configTreeWdog_RemoveKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _UNLOCK
    handlerRef = (configTreeWdog_KickEventHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);

    // Define storage for output parameters

    // Call the function
    configTreeWdog_RemoveKickEventHandler ( 
        handlerRef );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_configTreeWdog_AddKickEventHandler : Handle_configTreeWdog_AddKickEventHandler(msgRef); break;
        case _MSGID_configTreeWdog_RemoveKickEventHandler : Handle_configTreeWdog_RemoveKickEventHandler(msgRef); break;
