...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("logFd_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("logFd_ServerHandlers", 3);
...
static void Handle_logFd_StdErr
(
...
static void Handle_logFd_StdOut
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_logFd_StdErr : Handle_logFd_StdErr(msgRef); break;
        case _MSGID_logFd_StdOut : Handle_logFd_StdOut(msgRef); break;
