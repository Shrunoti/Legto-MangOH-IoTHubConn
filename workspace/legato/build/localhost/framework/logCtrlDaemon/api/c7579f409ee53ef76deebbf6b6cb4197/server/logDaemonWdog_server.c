...
#include "logDaemonWdog_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("logDaemonWdog_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("logDaemonWdog_ServerHandlers", 3);
...
static void AsyncResponse_logDaemonWdog_AddKickEventHandler
(
    void* contextPtr
)
{
...
static void Handle_logDaemonWdog_AddKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = logDaemonWdog_AddKickEventHandler ( 
        interval, AsyncResponse_logDaemonWdog_AddKickEventHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)logDaemonWdog_RemoveKickEventHandler;
...
static void Handle_logDaemonWdog_RemoveKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _UNLOCK
    handlerRef = (logDaemonWdog_KickEventHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);

    // Define storage for output parameters

    // Call the function
    logDaemonWdog_RemoveKickEventHandler ( 
        handlerRef );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_logDaemonWdog_AddKickEventHandler : Handle_logDaemonWdog_AddKickEventHandler(msgRef); break;
        case _MSGID_logDaemonWdog_RemoveKickEventHandler : Handle_logDaemonWdog_RemoveKickEventHandler(msgRef); break;
