...
#include "updateDaemonWdog_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("updateDaemonWdog_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("updateDaemonWdog_ServerHandlers", 3);
...
static void AsyncResponse_updateDaemonWdog_AddKickEventHandler
(
    void* contextPtr
)
{
...
static void Handle_updateDaemonWdog_AddKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = updateDaemonWdog_AddKickEventHandler ( 
        interval, AsyncResponse_updateDaemonWdog_AddKickEventHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)updateDaemonWdog_RemoveKickEventHandler;
...
static void Handle_updateDaemonWdog_RemoveKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _UNLOCK
    handlerRef = (updateDaemonWdog_KickEventHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);

    // Define storage for output parameters

    // Call the function
    updateDaemonWdog_RemoveKickEventHandler ( 
        handlerRef );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_updateDaemonWdog_AddKickEventHandler : Handle_updateDaemonWdog_AddKickEventHandler(msgRef); break;
        case _MSGID_updateDaemonWdog_RemoveKickEventHandler : Handle_updateDaemonWdog_RemoveKickEventHandler(msgRef); break;
