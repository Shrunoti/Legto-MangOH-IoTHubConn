...
#include "le_ima_messages.h"
#include "le_ima_interface.h"
...
typedef struct
{
    ...
    le_ima_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_ima_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_ima_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_ima_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_ima_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_ima disconnected\n");
}
...
)
{
    ...
    if ( strnlen(certPath, 511) > 511 )
    {
        LE_FATAL("strnlen(certPath, 511) > 511");
    }
    ...
    _msgPtr->id = _MSGID_le_ima_ImportCert;
