...
#include "supervisorWdog_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("supervisorWdog_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("supervisorWdog_ServerHandlers", 3);
...
static void AsyncResponse_supervisorWdog_AddKickEventHandler
(
    void* contextPtr
)
{
...
static void Handle_supervisorWdog_AddKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = supervisorWdog_AddKickEventHandler ( 
        interval, AsyncResponse_supervisorWdog_AddKickEventHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)supervisorWdog_RemoveKickEventHandler;
...
static void Handle_supervisorWdog_RemoveKickEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _UNLOCK
    handlerRef = (supervisorWdog_KickEventHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);

    // Define storage for output parameters

    // Call the function
    supervisorWdog_RemoveKickEventHandler ( 
        handlerRef );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_supervisorWdog_AddKickEventHandler : Handle_supervisorWdog_AddKickEventHandler(msgRef); break;
        case _MSGID_supervisorWdog_RemoveKickEventHandler : Handle_supervisorWdog_RemoveKickEventHandler(msgRef); break;
