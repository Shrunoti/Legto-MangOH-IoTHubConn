...
#include "le_appInfo_messages.h"
#include "le_appInfo_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_appInfo_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_appInfo_ServerHandlers", 3);
...
static void Handle_le_appInfo_GetState
(
...
)
{
    ...
    // Call the function
    le_appInfo_State_t _result;
    _result  = le_appInfo_GetState ( 
        appName );
...
static void Handle_le_appInfo_GetProcState
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_appInfo_ProcState_t _result;
    _result  = le_appInfo_GetProcState ( 
        appName, 
        procName );
...
static void Handle_le_appInfo_GetName
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    if ( (appNameSize > 47) &&
         (appNameSize < UINT32_MAX) )
    {
        LE_DEBUG("Adjusting appNameSize from %zu to 47", appNameSize);
        appNameSize = 47;
    }
    if (appNameSize >= UINT32_MAX)
    {
        appNameSize = 0;
    }
    else
    ...
    // Define storage for output parameters
    char appNameBuffer[48];
    char *appName = appNameBuffer;
    appName[0] = 0;
...
static void Handle_le_appInfo_GetHash
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_appInfo_GetState : Handle_le_appInfo_GetState(msgRef); break;
        case _MSGID_le_appInfo_GetProcState : Handle_le_appInfo_GetProcState(msgRef); break;
        case _MSGID_le_appInfo_GetName : Handle_le_appInfo_GetName(msgRef); break;
        case _MSGID_le_appInfo_GetHash : Handle_le_appInfo_GetHash(msgRef); break;
