...
#include "le_dualsys_messages.h"
#include "le_dualsys_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_dualsys_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_dualsys_ServerHandlers", 3);
...
static void Handle_le_dualsys_DisableSyncBeforeUpdate
(
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_dualsys_DisableSyncBeforeUpdate ( 
        isDisabled );
...
static void Handle_le_dualsys_SetSystem
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_dualsys_GetCurrentSystem
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    le_dualsys_System_t systemMaskBuffer;
    le_dualsys_System_t *systemMaskPtr = &systemMaskBuffer;
    ...
    // Call the function
    le_result_t _result;
    _result  = le_dualsys_GetCurrentSystem ( 
        systemMaskPtr );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_dualsys_DisableSyncBeforeUpdate : Handle_le_dualsys_DisableSyncBeforeUpdate(msgRef); break;
        case _MSGID_le_dualsys_SetSystem : Handle_le_dualsys_SetSystem(msgRef); break;
        case _MSGID_le_dualsys_GetCurrentSystem : Handle_le_dualsys_GetCurrentSystem(msgRef); break;
