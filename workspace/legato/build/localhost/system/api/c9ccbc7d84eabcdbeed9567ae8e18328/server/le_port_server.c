...
#include "le_port_messages.h"
#include "le_port_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_port_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_port_ServerHandlers", 3);
...
static void Handle_le_port_Request
(
...
)
{
    ...
    // Call the function
    le_port_DeviceRef_t _result;
    _result  = le_port_Request ( 
        deviceName );
...
static void Handle_le_port_SetDataMode
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_port_SetCommandMode
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    le_atServer_DeviceRef_t atServerDevRefBuffer;
    le_atServer_DeviceRef_t *atServerDevRefPtr = &atServerDevRefBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        atServerDevRefPtr = NULL;
    }

    // Call the function
    le_result_t _result;
    _result  = le_port_SetCommandMode ( 
        devRef, 
        atServerDevRefPtr );
...
static void Handle_le_port_Release
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_port_GetPortReference
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    le_port_DeviceRef_t devRefBuffer;
    le_port_DeviceRef_t *devRefPtr = &devRefBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        devRefPtr = NULL;
    }
    ...
    // Call the function
    le_result_t _result;
    _result  = le_port_GetPortReference ( 
        atServerDevRef, 
        devRefPtr );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_port_Request : Handle_le_port_Request(msgRef); break;
        case _MSGID_le_port_SetDataMode : Handle_le_port_SetDataMode(msgRef); break;
        case _MSGID_le_port_SetCommandMode : Handle_le_port_SetCommandMode(msgRef); break;
        case _MSGID_le_port_Release : Handle_le_port_Release(msgRef); break;
        case _MSGID_le_port_GetPortReference : Handle_le_port_GetPortReference(msgRef); break;
