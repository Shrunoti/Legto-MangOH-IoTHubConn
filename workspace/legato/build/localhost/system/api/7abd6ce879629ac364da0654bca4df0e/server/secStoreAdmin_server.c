...
#include "secStoreAdmin_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("secStoreAdmin_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("secStoreAdmin_ServerHandlers", 3);
...
static void Handle_secStoreAdmin_CreateIter
(
...
)
{
    ...
    // Call the function
    secStoreAdmin_IterRef_t _result;
    _result  = secStoreAdmin_CreateIter ( 
        path );
...
static void Handle_secStoreAdmin_DeleteIter
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_secStoreAdmin_Next
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_secStoreAdmin_GetEntry
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = secStoreAdmin_GetEntry ( 
        iterRef, 
        name, 
        nameSize, 
        isDirPtr );
...
static void Handle_secStoreAdmin_Write
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_secStoreAdmin_Read
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    if ( (bufSize > 8192) )
    {
        LE_DEBUG("Adjusting bufSize from %zu to 8192", bufSize);
        bufSize = 8192;
    }

    // Define storage for output parameters
    uint8_t bufBuffer[8192];
...
static void Handle_secStoreAdmin_CopyMetaTo
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = secStoreAdmin_CopyMetaTo ( 
        path );
...
static void Handle_secStoreAdmin_Delete
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_secStoreAdmin_GetSize
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_secStoreAdmin_GetTotalSpace
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    uint64_t totalSizeBuffer;
    uint64_t *totalSizePtr = &totalSizeBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        totalSizePtr = NULL;
    }
    uint64_t freeSizeBuffer;
    uint64_t *freeSizePtr = &freeSizeBuffer;
    if (!(_requiredOutputs & (1u << 1)))
    {
        freeSizePtr = NULL;
    }
    ...
    // Call the function
    le_result_t _result;
    _result  = secStoreAdmin_GetTotalSpace ( 
        totalSizePtr, 
        freeSizePtr );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_secStoreAdmin_CreateIter : Handle_secStoreAdmin_CreateIter(msgRef); break;
        case _MSGID_secStoreAdmin_DeleteIter : Handle_secStoreAdmin_DeleteIter(msgRef); break;
        case _MSGID_secStoreAdmin_Next : Handle_secStoreAdmin_Next(msgRef); break;
        case _MSGID_secStoreAdmin_GetEntry : Handle_secStoreAdmin_GetEntry(msgRef); break;
        case _MSGID_secStoreAdmin_Write : Handle_secStoreAdmin_Write(msgRef); break;
        case _MSGID_secStoreAdmin_Read : Handle_secStoreAdmin_Read(msgRef); break;
        case _MSGID_secStoreAdmin_CopyMetaTo : Handle_secStoreAdmin_CopyMetaTo(msgRef); break;
        case _MSGID_secStoreAdmin_Delete : Handle_secStoreAdmin_Delete(msgRef); break;
        case _MSGID_secStoreAdmin_GetSize : Handle_secStoreAdmin_GetSize(msgRef); break;
        case _MSGID_secStoreAdmin_GetTotalSpace : Handle_secStoreAdmin_GetTotalSpace(msgRef); break;
