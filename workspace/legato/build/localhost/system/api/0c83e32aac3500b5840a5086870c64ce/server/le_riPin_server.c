...
#include "le_riPin_messages.h"
#include "le_riPin_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_riPin_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_riPin_ServerHandlers", 3);
...
static void Handle_le_riPin_AmIOwnerOfRingSignal
(
...
)
{
    ...
    bool amIOwnerBuffer;
    bool *amIOwnerPtr = &amIOwnerBuffer;
    ...
    // Call the function
    le_result_t _result;
    _result  = le_riPin_AmIOwnerOfRingSignal ( 
        amIOwnerPtr );
...
static void Handle_le_riPin_TakeRingSignal
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_riPin_TakeRingSignal (  );
...
static void Handle_le_riPin_ReleaseRingSignal
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_riPin_ReleaseRingSignal (  );
...
static void Handle_le_riPin_PulseRingSignal
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_riPin_AmIOwnerOfRingSignal : Handle_le_riPin_AmIOwnerOfRingSignal(msgRef); break;
        case _MSGID_le_riPin_TakeRingSignal : Handle_le_riPin_TakeRingSignal(msgRef); break;
        case _MSGID_le_riPin_ReleaseRingSignal : Handle_le_riPin_ReleaseRingSignal(msgRef); break;
        case _MSGID_le_riPin_PulseRingSignal : Handle_le_riPin_PulseRingSignal(msgRef); break;
