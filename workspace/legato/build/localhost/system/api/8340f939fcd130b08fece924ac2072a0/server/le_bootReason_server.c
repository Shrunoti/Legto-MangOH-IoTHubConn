...
#include "le_bootReason_messages.h"
#include "le_bootReason_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_bootReason_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_bootReason_ServerHandlers", 3);
...
static void Handle_le_bootReason_WasTimer
(
...
)
{
    ...
    // Call the function
    bool _result;
    _result  = le_bootReason_WasTimer (  );
...
static void Handle_le_bootReason_WasGpio
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_bootReason_WasAdc
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_bootReason_WasTimer : Handle_le_bootReason_WasTimer(msgRef); break;
        case _MSGID_le_bootReason_WasGpio : Handle_le_bootReason_WasGpio(msgRef); break;
        case _MSGID_le_bootReason_WasAdc : Handle_le_bootReason_WasAdc(msgRef); break;
