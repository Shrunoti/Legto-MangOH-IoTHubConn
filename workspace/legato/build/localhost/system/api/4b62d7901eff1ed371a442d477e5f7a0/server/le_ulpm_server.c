...
#include "le_ulpm_messages.h"
#include "le_ulpm_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_ulpm_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_ulpm_ServerHandlers", 3);
...
static void Handle_le_ulpm_BootOnGpio
(
...
static void Handle_le_ulpm_BootOnTimer
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_ulpm_BootOnAdc
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_ulpm_GetFirmwareVersion
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    {
        LE_DEBUG("Adjusting versionSize from %zu to 8", versionSize);
        versionSize = 8;
    }
    ...
    // Call the function
    le_result_t _result;
    _result  = le_ulpm_GetFirmwareVersion ( 
        version, 
        versionSize );
...
static void Handle_le_ulpm_ShutDown
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_ulpm_BootOnGpio : Handle_le_ulpm_BootOnGpio(msgRef); break;
        case _MSGID_le_ulpm_BootOnTimer : Handle_le_ulpm_BootOnTimer(msgRef); break;
        case _MSGID_le_ulpm_BootOnAdc : Handle_le_ulpm_BootOnAdc(msgRef); break;
        case _MSGID_le_ulpm_GetFirmwareVersion : Handle_le_ulpm_GetFirmwareVersion(msgRef); break;
        case _MSGID_le_ulpm_ShutDown : Handle_le_ulpm_ShutDown(msgRef); break;
