...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_temp_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_temp_ServerHandlers", 3);
...
)
{
    ...
    {
        LE_DEBUG("Adjusting sensorNameSize from %zu to 100", sensorNameSize);
        sensorNameSize = 100;
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_temp_Request : Handle_le_temp_Request(msgRef); break;
        case _MSGID_le_temp_GetSensorName : Handle_le_temp_GetSensorName(msgRef); break;
        case _MSGID_le_temp_AddThresholdEventHandler : Handle_le_temp_AddThresholdEventHandler(msgRef); break;
        case _MSGID_le_temp_RemoveThresholdEventHandler : Handle_le_temp_RemoveThresholdEventHandler(msgRef); break;
        case _MSGID_le_temp_GetTemperature : Handle_le_temp_GetTemperature(msgRef); break;
        case _MSGID_le_temp_SetThreshold : Handle_le_temp_SetThreshold(msgRef); break;
        case _MSGID_le_temp_GetThreshold : Handle_le_temp_GetThreshold(msgRef); break;
        case _MSGID_le_temp_StartMonitoring : Handle_le_temp_StartMonitoring(msgRef); break;
