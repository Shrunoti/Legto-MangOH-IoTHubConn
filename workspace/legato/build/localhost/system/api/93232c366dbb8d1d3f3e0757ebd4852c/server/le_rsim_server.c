...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_rsim_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_rsim_ServerHandlers", 3);
...
static void AsyncResponse_le_rsim_AddMessageHandler
(
...
static void Handle_le_rsim_AddMessageHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = le_rsim_AddMessageHandler ( AsyncResponse_le_rsim_AddMessageHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)le_rsim_RemoveMessageHandler;
...
static void Handle_le_rsim_RemoveMessageHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_rsim_MessageHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);
...
static void AsyncResponse_le_rsim_SendMessage
(
    uint8_t messageId,
    le_result_t result,
    void* contextPtr
)
{
    ...
    {
        LE_FATAL("Handler passed to le_rsim_SendMessage() can't be called more than once");
    }
...
static void Handle_le_rsim_SendMessage
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_rsim_SendMessage ( 
        messagePtr, 
        messageSize, AsyncResponse_le_rsim_SendMessage, 
        contextPtr );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_rsim_AddMessageHandler : Handle_le_rsim_AddMessageHandler(msgRef); break;
        case _MSGID_le_rsim_RemoveMessageHandler : Handle_le_rsim_RemoveMessageHandler(msgRef); break;
        case _MSGID_le_rsim_SendMessage : Handle_le_rsim_SendMessage(msgRef); break;
