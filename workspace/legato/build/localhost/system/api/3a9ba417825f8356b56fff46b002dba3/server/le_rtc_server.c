...
#include "le_rtc_messages.h"
#include "le_rtc_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_rtc_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_rtc_ServerHandlers", 3);
...
static void Handle_le_rtc_GetUserTime
(
...
)
{
    ...
    uint64_t millisecondsPastGpsEpochBuffer;
    uint64_t *millisecondsPastGpsEpochPtr = &millisecondsPastGpsEpochBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        millisecondsPastGpsEpochPtr = NULL;
    }

    // Call the function
    le_result_t _result;
    _result  = le_rtc_GetUserTime ( 
        millisecondsPastGpsEpochPtr );
    ...
    if (millisecondsPastGpsEpochPtr)
    {
...
static void Handle_le_rtc_SetUserTime
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_rtc_GetUserTime : Handle_le_rtc_GetUserTime(msgRef); break;
        case _MSGID_le_rtc_SetUserTime : Handle_le_rtc_SetUserTime(msgRef); break;
