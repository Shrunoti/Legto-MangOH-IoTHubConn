...
#ifndef LE_GPIOPIN2_INTERFACE_H_INCLUDE_GUARD
#define LE_GPIOPIN2_INTERFACE_H_INCLUDE_GUARD
...
typedef enum
{
    LE_GPIOPIN2_ACTIVE_HIGH = 0,
        ///< GPIO active-high, output is 1
    LE_GPIOPIN2_ACTIVE_LOW = 1
        ///< GPIO active-low, output is 0
}
le_gpioPin2_Polarity_t;
...
typedef enum
{
    LE_GPIOPIN2_EDGE_NONE = 0,
        ///< No edge detection
    LE_GPIOPIN2_EDGE_RISING = 1,
        ///< Notify when voltage goes from low to high.
    LE_GPIOPIN2_EDGE_FALLING = 2,
        ///< Notify when voltage goes from high to low.
    LE_GPIOPIN2_EDGE_BOTH = 3
...
//--------------------------------------------------------------------------------------------------
/**
 * Reference type used by Add/Remove functions for EVENT 'le_gpioPin2_ChangeEvent'
 */
//--------------------------------------------------------------------------------------------------
typedef struct le_gpioPin2_ChangeEventHandler* le_gpioPin2_ChangeEventHandlerRef_t;
...
typedef enum
{
    LE_GPIOPIN2_PULL_OFF = 0,
        ///< Neither resistor is enabled
    LE_GPIOPIN2_PULL_DOWN = 1,
        ///< Pulldown resistor is enabled
    LE_GPIOPIN2_PULL_UP = 2
        ///< Pullup resistor is enabled
}
le_gpioPin2_PullUpDown_t;
 ...
 */
//--------------------------------------------------------------------------------------------------
typedef void (*le_gpioPin2_ChangeCallbackFunc_t)
(
    bool state,
...
//--------------------------------------------------------------------------------------------------
/**
 * Add handler function for EVENT 'le_gpioPin2_ChangeEvent'
 *
...
//--------------------------------------------------------------------------------------------------
/**
 * Remove handler function for EVENT 'le_gpioPin2_ChangeEvent'
 */
//--------------------------------------------------------------------------------------------------
void le_gpioPin2_RemoveChangeEventHandler
(
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t le_gpioPin2_SetEdgeSense
(
    le_gpioPin2_Edge_t trigger
        ///< [IN] Change(s) that should trigger the callback to be called.
);
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t le_gpioPin2_DisableEdgeSense
(
    void
);
 ...
 */
//--------------------------------------------------------------------------------------------------
le_gpioPin2_Edge_t le_gpioPin2_GetEdgeSense
(
    void
);
 ...
 */
//--------------------------------------------------------------------------------------------------
le_gpioPin2_Polarity_t le_gpioPin2_GetPolarity
(
    void
);
 ...
 */
//--------------------------------------------------------------------------------------------------
le_gpioPin2_PullUpDown_t le_gpioPin2_GetPullUpDown
(
    void
);


#endif // LE_GPIOPIN2_INTERFACE_H_INCLUDE_GUARD