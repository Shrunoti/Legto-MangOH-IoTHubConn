...
typedef struct
{
    ...
    le_cellnet_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL("Component for le_cellnet disconnected\n");
}
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_cellnet_StateEventHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(pinCode, 8) > 8 )
    {
        LE_FATAL("strnlen(pinCode, 8) > 8");
    }
...
)
{
    ...
    if (pinCode)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (pinCodeSize-1) ));
    }
