...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_cellnet_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_cellnet_ServerHandlers", 3);
...
static void AsyncResponse_le_cellnet_AddStateEventHandler
(
...
static void Handle_le_cellnet_AddStateEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _result  = le_cellnet_AddStateEventHandler ( AsyncResponse_le_cellnet_AddStateEventHandler, 
        contextPtr );
    ...
    if (_result)
    {
        ...
        serverDataPtr->removeHandlerFunc =
            (RemoveHandlerFunc_t)le_cellnet_RemoveStateEventHandler;
...
static void Handle_le_cellnet_RemoveStateEventHandler
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_cellnet_StateEventHandlerRef_t)serverDataPtr->handlerRef;
    le_mem_Release(serverDataPtr);
...
static void Handle_le_cellnet_Request
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_cellnet_Release
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_cellnet_SetSimPinCode
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_cellnet_SetSimPinCode ( 
        simId, 
        pinCode );
...
static void Handle_le_cellnet_GetSimPinCode
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    if ( (pinCodeSize > 8) &&
         (pinCodeSize < UINT32_MAX) )
    {
        LE_DEBUG("Adjusting pinCodeSize from %zu to 8", pinCodeSize);
        pinCodeSize = 8;
    }
    if (pinCodeSize >= UINT32_MAX)
    {
        pinCodeSize = 0;
    }
    else
    ...
    // Define storage for output parameters
    char pinCodeBuffer[9];
    char *pinCode = pinCodeBuffer;
    pinCode[0] = 0;
    ...
    // Call the function
    le_result_t _result;
    _result  = le_cellnet_GetSimPinCode ( 
        simId, 
        pinCode, 
        pinCodeSize );
...
static void Handle_le_cellnet_GetNetworkState
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    le_cellnet_State_t stateBuffer;
    le_cellnet_State_t *statePtr = &stateBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        statePtr = NULL;
    }
    ...
    // Call the function
    le_result_t _result;
    _result  = le_cellnet_GetNetworkState ( 
        statePtr );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_cellnet_AddStateEventHandler : Handle_le_cellnet_AddStateEventHandler(msgRef); break;
        case _MSGID_le_cellnet_RemoveStateEventHandler : Handle_le_cellnet_RemoveStateEventHandler(msgRef); break;
        case _MSGID_le_cellnet_Request : Handle_le_cellnet_Request(msgRef); break;
        case _MSGID_le_cellnet_Release : Handle_le_cellnet_Release(msgRef); break;
        case _MSGID_le_cellnet_SetSimPinCode : Handle_le_cellnet_SetSimPinCode(msgRef); break;
        case _MSGID_le_cellnet_GetSimPinCode : Handle_le_cellnet_GetSimPinCode(msgRef); break;
        case _MSGID_le_cellnet_GetNetworkState : Handle_le_cellnet_GetNetworkState(msgRef); break;
