...
typedef struct
{
    ...
    le_secStore_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_secStore_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_secStore_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_secStore_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_secStore_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_secStore disconnected\n");
}
...
)
{
    ...
    if (bufPtr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (*bufSizePtr) ));
    }
