...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_secStore_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_secStore_ServerHandlers", 3);
...
static void Handle_le_secStore_Write
(
...
static void Handle_le_secStore_Read
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_le_secStore_Delete
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_secStore_Write : Handle_le_secStore_Write(msgRef); break;
        case _MSGID_le_secStore_Read : Handle_le_secStore_Read(msgRef); break;
        case _MSGID_le_secStore_Delete : Handle_le_secStore_Delete(msgRef); break;
