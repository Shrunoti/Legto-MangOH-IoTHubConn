...
#include "secStoreGlobal_messages.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("secStoreGlobal_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("secStoreGlobal_ServerHandlers", 3);
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_secStoreGlobal_Write : Handle_secStoreGlobal_Write(msgRef); break;
        case _MSGID_secStoreGlobal_Read : Handle_secStoreGlobal_Read(msgRef); break;
        case _MSGID_secStoreGlobal_Delete : Handle_secStoreGlobal_Delete(msgRef); break;
