...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_fwupdate_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_fwupdate_ServerHandlers", 3);
...
)
{
    ...
    if (positionPtr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize,
                                                      *positionPtr ));
    }
...
)
{
    ...
    {
        LE_DEBUG("Adjusting statusLabelSize from %zu to 32", statusLabelSize);
        statusLabelSize = 32;
    }
    ...
    // Define storage for output parameters
    le_fwupdate_UpdateStatus_t statusBuffer;
    le_fwupdate_UpdateStatus_t *statusPtr = &statusBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        statusPtr = NULL;
    }
    char statusLabelBuffer[33];
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_fwupdate_Download : Handle_le_fwupdate_Download(msgRef); break;
        case _MSGID_le_fwupdate_InitDownload : Handle_le_fwupdate_InitDownload(msgRef); break;
        case _MSGID_le_fwupdate_GetResumePosition : Handle_le_fwupdate_GetResumePosition(msgRef); break;
        case _MSGID_le_fwupdate_GetUpdateStatus : Handle_le_fwupdate_GetUpdateStatus(msgRef); break;
        case _MSGID_le_fwupdate_GetFirmwareVersion : Handle_le_fwupdate_GetFirmwareVersion(msgRef); break;
        case _MSGID_le_fwupdate_GetBootloaderVersion : Handle_le_fwupdate_GetBootloaderVersion(msgRef); break;
        case _MSGID_le_fwupdate_GetAppBootloaderVersion : Handle_le_fwupdate_GetAppBootloaderVersion(msgRef); break;
        case _MSGID_le_fwupdate_IsSystemMarkedGood : Handle_le_fwupdate_IsSystemMarkedGood(msgRef); break;
        case _MSGID_le_fwupdate_Install : Handle_le_fwupdate_Install(msgRef); break;
        case _MSGID_le_fwupdate_MarkGood : Handle_le_fwupdate_MarkGood(msgRef); break;
        case _MSGID_le_fwupdate_InstallAndMarkGood : Handle_le_fwupdate_InstallAndMarkGood(msgRef); break;
