...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_adc_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_adc_ServerHandlers", 3);
...
static void Handle_le_adc_ReadValue
(
...
)
{
    ...
    int32_t adcValueBuffer;
    int32_t *adcValuePtr = &adcValueBuffer;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_adc_ReadValue : Handle_le_adc_ReadValue(msgRef); break;
