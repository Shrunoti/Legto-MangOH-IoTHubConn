...
#include "le_instStat_messages.h"
...
typedef struct
{
    ...
    le_instStat_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_instStat_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_instStat_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_instStat_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_instStat_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_instStat disconnected\n");
}
...
static void _Handle_le_instStat_AddAppInstallEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_instStat_AppInstallHandlerFunc_t _handlerRef_le_instStat_AddAppInstallEventHandler = (le_instStat_AppInstallHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_le_instStat_AddAppInstallEventHandler != NULL )
    {
        _handlerRef_le_instStat_AddAppInstallEventHandler(appName, contextPtr );
    }
    else
    {
...
)
{
    ...
    le_instStat_AppInstallEventHandlerRef_t _result;
    ...
    _msgPtr->id = _MSGID_le_instStat_AddAppInstallEventHandler;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_instStat_RemoveAppInstallEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (le_instStat_AppInstallEventHandlerRef_t)clientDataPtr->handlerRef;
...
// This function parses the message buffer received from the server, and then calls the user
// registered handler, which is stored in a client data object.
static void _Handle_le_instStat_AddAppUninstallEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    // Pull out additional data from the client data pointer
    le_instStat_AppUninstallHandlerFunc_t _handlerRef_le_instStat_AddAppUninstallEventHandler = (le_instStat_AppUninstallHandlerFunc_t)_clientDataPtr->handlerPtr;
    void* contextPtr = _clientDataPtr->contextPtr;
    ...
    // Call the registered handler
    if ( _handlerRef_le_instStat_AddAppUninstallEventHandler != NULL )
    {
        _handlerRef_le_instStat_AddAppUninstallEventHandler(appName, contextPtr );
    }
    else
    {
        LE_FATAL("Error in client data: no registered handler");
...
)
{
    ...
    le_instStat_AppUninstallEventHandlerRef_t _result;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_instStat_AddAppUninstallEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_instStat_RemoveAppUninstallEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    le_ref_DeleteRef(_HandlerRefMap, handlerRef);
    _UNLOCK
    handlerRef = (le_instStat_AppUninstallEventHandlerRef_t)clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr, &_msgBufSize,
                                     handlerRef ));
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_instStat_AddAppInstallEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_le_instStat_AddAppInstallEventHandler, msgRef, clientDataPtr);
            break;
        case _MSGID_le_instStat_AddAppUninstallEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_le_instStat_AddAppUninstallEventHandler, msgRef, clientDataPtr);
            break;
